...
class Bootstrap extends Zend_Application_Bootstrap_Bootstrap
{
	...
	protected function _initDatabase(){
		...
		Zend_Registry::set("database", $config['resources']['db']['params']['dbname']);
	...
	protected function _initAutoload()
    {
		...
		if (strpos($oldurl, 'postdetail') == '')
		{
			...
			if ($rsTemp){
				...
				header("Location: " . 'http://liveindia.live/postdetail/index/id/'.$rsTemp['post_id'].'/'.$rsTemp['english_url'].'/');
				exit;
			} else {
...
function getMetaDetail(){
	$title = $metaDescription = $metaKeywords = $url = '';
	...
	if ( strpos($pageurl, 'postdetail')){
		...
		if ($rsTemp){
			...
			$image=$rsTemp['image'];
			if (!empty($rsTemp['headline'])){
				$title = $rsTemp['headline'];
			} else {
				$title = $rsTemp['meta_title'];
			}
		
			if (!empty($rsTemp['intro'])){
		...
		if ($rsTemp){
			$url = "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];
			$title = $rsTemp['category'] . " Hindi News, Latest ". $rsTemp['category'] ." Hindi Samachar (Khabar) from World";
			$metaDescription = "Live India- Get Live ". $rsTemp['category'] ." Hindi News from India and Around the World, Read and share ". $rsTemp['category'] ." Latest and interesting Hindi Samachar (Khabar) with your Social Friends, Best Selected ". $rsTemp['category'] ." Hindi Khabar.";
			$metaKeywords = $rsTemp['category'] ." Khabar, ". $rsTemp['category'] ." Hindi Khabar, ". $rsTemp['category'] ." Samachar, ". $rsTemp['category'] ." Latest Khabar, ". $rsTemp['category'] ." India Khabar, ". $rsTemp['category'] ." World Khabar Live, Live Hindi News";
		}
	} else {
...
<?php if( true == isset( $image ) ){ ?>
    <meta property="og:image" content="http://<?php echo $_SERVER['HTTP_HOST'] . $GLOBALS['baseUrl'];?>/public/products/image/<?php echo $image;?>" >
    <?php } ?>
    ...
    <meta itemprop="keywords" content="<?php echo $metaKeywords;?>" />
     <?php if (!empty($image)) {?>
    	<img src="http://<?php echo $_SERVER['HTTP_HOST'] . $GLOBALS['baseUrl'];?>/public/products/image/<?php echo $image;?>" width="1" height="1" style="display:none;" alt="<?php echo $title;?>" title="<?php echo $title;?>"/>
        <link rel="image_src" href="http://<?php echo $_SERVER['HTTP_HOST'] . $GLOBALS['baseUrl'];?>/public/products/image/<?php echo $image;?>" />
    <?php } else {?>
    	<img src="http://<?php echo $_SERVER['HTTP_HOST'] . $GLOBALS['baseUrl'];?>/public/images/footer-logo.png" width="1" height="1" style="display:none;" alt="<?php echo $title;?>" title="<?php echo $title;?>"/>
        <link rel="image_src" href="http://<?php echo $_SERVER['HTTP_HOST'] . $GLOBALS['baseUrl'];?>/public/images/footer-logo.png" />
    <?php }?>
<?php
}
...
function getMainBodyCategoryWisePost($position, $category){
	...
	{ 
		//foreach($rsCat as $rsCatVal)
		//{
			$mySql  = "SELECT P.post_id, CONCAT(SUBSTRING_INDEX(P.headline, ' ', 12), '...') AS headline, P.image, P.english_url, P.user_name FROM post P";
		    $mySql .= " JOIN post_category PC ON P.post_id = PC.post_id";
		    $mySql .= " WHERE category_id = ".$rsCat['category_id'];
			$mySql .= " AND P.status = 1";
			$mySql .= " AND P.sectionpriority IS NOT NULL";
			$mySql .= " ORDER BY P.post_id DESC, P.sectionpriority";
			$mySql .= " LIMIT 6";
			//echo $mySql;
			//exit;
			$rsTemp = Zend_Registry::get("db")->fetchAll($mySql);
...
?>
				<div class="cp-news-grid-style-4 mb20">
                    <div class="section-title pink-border">
                      <h2><a href="<?php echo $GLOBALS['baseUrl'].'/category/index/id/'.$rsCat['category_id'].'/'.$rsCat['category'];?>" class="textpink"><?php echo $rsCat['category'];?></a></h2>
                      <small></small> </div>
...
function getGallery(){
	if (basename($_SERVER['REQUEST_URI']) == 'liveindialive'
		...
		$mySql  = "SELECT G.idgallery, G.title, G.englishurl, GI.image FROM gallery G JOIN galleryimage GI ON G.idgallery = GI.idgallery";
		...
		{ 
?>
            <div id="hideonmobile" class="cp-news-grid-style-3 m20">
                <div class="section-title pink-border">
                                ...
                                <li class="col-md-12 col-sm-12 cp-news-post">
                                    <div class="cp-thumb">
                                        ...
                                        <div class="textcenter pad5" style="position:absolute; top:5px;">
                                            <span class="textcenter"><a href="<?php echo $GLOBALS['baseUrl']?>/gallery/index/id/<?php echo $rsTemp['idgallery'];?>/<?php echo $rsTemp['englishurl'];?>/" class="textpink text21"><?php echo $rsTemp['title'];?></a></span>
                                        </div>
                                        <div class="textcenter pad5" style="position:absolute; bottom:5px;">
                                        <a href="<?php echo $GLOBALS['baseUrl']?>/gallery/index/id/<?php echo $rsTemp['idgallery'];?>/<?php echo $rsTemp['englishurl'];?>/"><img class="lazy" src="<?php echo $GLOBALS['baseUrl']?>/public/images/gallaryplay.png" style="width:50px;" /></a>
                                        </div>
                                    </div>
                                    <?php
                                    /*
...
function getRightBodyCategoryWisePost($position, $category){
	...
	{ 
		//foreach($rsCat as $rsCatVal)
		//{
			$mySql  = "SELECT P.post_id, CONCAT(SUBSTRING_INDEX(P.headline, ' ', 8), '...') AS headline, CONCAT(SUBSTRING_INDEX(P.intro, ' ', 10), '...') AS intro, P.image, P.english_url FROM post P JOIN post_category PC ON P.post_id = PC.post_id";
			$mySql .= " WHERE status = 1";
...
?>
                    ...
                    <div style="text-align:center; margin:20px 0;">
					   <a href="<?php echo $GLOBALS['baseUrl'];?>/astrology/"><img src="<?php echo $GLOBALS['baseUrl'];?>/public/images/astrology-header1.jpg" border="0" style="width:100%;" /></a>
                    </div>
				...
				?>
                        <div class="widget latest-reviews">
                            <div class="section-title pink-border" style="margin-bottom:0px;">
                                <h2><a href="<?php echo $GLOBALS['baseUrl'];?>/category/index/id/<?php echo $rsCat['category_id'];?>/<?php echo $rsCat['category'];?>" class="textpink"><?php echo $rsCat['category'];?></a></h2>
                                <small></small> 
                            </div>
                            <div class="cp-sidebar-content">
                                <ul class="reviews">
...
function getMainBodyCategoryWisePostOnDetailPage($position, $category){
	$mySql = "SELECT category_id, category FROM category WHERE category = '".addslashes($category)."'";
	//$mySql .= "AND position = '".$position."'";
	$mySql .= " ORDER BY sort LIMIT 1";
	//echo $mySql;
	//exit;
	$rsCat= Zend_Registry::get("db")->fetchRow($mySql);
	...
	{ 
		//foreach($rsCat as $rsCatVal)
		//{
			$mySql  = "SELECT P.post_id, CONCAT(SUBSTRING_INDEX(P.headline, ' ', 12), '...') AS headline, P.image, P.english_url, P.user_name FROM post P JOIN post_category PC ON P.post_id = PC.post_id";
			$mySql .= " WHERE category_id = ".$rsCat['category_id'];
			$mySql .= " AND status = 1";
			$mySql .= " AND P.sectionpriority IS NOT NULL";
			$mySql .= " ORDER BY P.post_id DESC, P.sectionpriority";
