...
//default page of this controller
	...
	function addAction(){
		if($this->getRequest()->isPost()){
			...
			$position = addslashes($formData['position']);
			$sort = ($formData['sort']);
	...
	function editAction(){
		...
		//echo $adminid;die();
		$sql="SELECT * FROM `category` WHERE parentId = 0 AND `category_id` = '".$category_id."'";
		//echo $sql;
		//exit;
		...
		if($this->getRequest()->isPost()){
			...
			$mySql .= " AND category_id <> '".$category_id."'";
			$rsTemp = Zend_Registry::get("db")->fetchRow($mySql);
			
			if($rsTemp){
				if($rsTemp['totRec'] > 0){
					$error['category']='Category already exist';	
				}
			}
