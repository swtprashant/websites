	...
	function addAction(){
		if($this->getRequest()->isPost()){
			...
			if(count($error)== 0){
				...
				if($category){
					$sql = "";
					$sql = "DELETE FROM `post_category` WHERE `post_id`= '".$lastinsertid."'";
					$rsTemp = Zend_Registry::get("db")->query($sql);
...
function setAutoPriority($start){
	if($start != ''){ 
		...
		{
			$mySQL = "";
			$mySQL .=" SELECT post_id FROM post WHERE `priority` = '".$i."';";
			//echo $mySQL .'<br>';
...
function replaceJunk($str){
	...
	$str = str_replace('<span lang="HI" style="font-size: 10pt; line-height: 115%; : , ;">','', $str);
	$str = str_replace('</span>','', $str);
	$str = str_replace(' class="MsoNormal"','', $str);
	$str = str_replace('<o:p>','', $str);
	$str = str_replace('</o:p>','', $str);
	$str = str_replace(' style=" margin:8px 0 0 0; padding:0 4px;"','', $str);
	$str = str_replace(' margin:8px 0 0 0;"','', $str);
	$str = str_replace(' padding:0 4px;"','', $str);
	$str = str_replace('<p class="MsoNormal">&nbsp;</p>','', $str);
	$str = str_replace('<p>&nbsp;</p>','', $str);
